        ...
        <v-layout row wrap>
            ...
            <v-flex sm5 md4 xs12>
                <v-card class="elevation-1 mb-3">
                    ...
                    <form action="{{ route('roadtrips.categories.store') }}" method="POST">
                        {{ csrf_field() }}
                        <v-card-text>
                            <input type="hidden" name="categorable_type" value="roadtrip">
                            <v-text-field
                            ...
                            <v-text-field
                                ...
                                hint="{{ __('Will be used as a unique category name.') }}"
                                label="{{ _('Code') }}"
                                name="code"
                            ></v-text-field>
                            <v-text-field
            ...
            <v-flex sm7 md8 xs12>
                <v-card class="mb-3 elevation-1">
                    <v-toolbar class="transparent elevation-0">
                        <v-toolbar-title class="accent--text">{{ __('Categories') }}</v-toolbar-title>
                        <v-spacer></v-spacer>
                        ...
                        <v-slide-y-transition>
                            <template v-if="dataset.selected.length > 1">
                                <form action="{{ route('categories.many.destroy') }}" method="POST" class="inline">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                    ...
                    >
                        ...
                        <template slot="items" scope="prop">
                            ...
                            <td width="100%">
                                <a :href="route(urls.categories.edit, prop.item.id)">
                            ...
                            <td class="text-xs-center">
                                <v-menu bottom left>
                                    ...
                                    <v-list>
                                        <v-list-tile :href="route(urls.categories.show, (prop.item.id))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile @click="post(route(urls.categories.api.clone, (prop.item.id)))">
                                            <v-list-tile-action>
                                        ...
                                        <v-list-tile
                                            @click="destroy(route(urls.categories.api.destroy, prop.item.id),
                                            {
                                                '_token': '{{ csrf_token() }}'
                                            })">
                                            <v-list-tile-action>
        ...
        mixins.push({
            data () {
                return {
                    ...
                    urls: {
                        categories: {
                            api: {
                                clone: '{{ route('api.roadtrips.categories.clone', 'null') }}',
                                destroy: '{{ route('api.roadtrips.categories.destroy', 'null') }}',
                            },
            ...
            watch: {
                ...
                'dataset.searchform.query': function (filter) {
                    setTimeout(() => {
                        ...
                        this.api().search('{{ route('api.roadtrips.categories.search') }}', query)
                            .then((data) => {
            ...
            methods: {
                get () {
                    ...
                    this.api().get('{{ route('api.roadtrips.categories.all') }}', query)
                        .then((data) => {
                            this.dataset.items = data.items.data ? data.items.data : data.items;
                            this.dataset.totalItems = data.items.total ? data.items.total : data.total;
                            this.dataset.loading = false;
                ...
                post (url, query) {
                    ...
                    this.api().post(url, query)
                        .then((data) => {
                            self.get('{{ route('api.roadtrips.categories.all') }}');
